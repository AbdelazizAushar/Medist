...
class MedicineController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if($request->searchBy == 'medicine') {
            $data = Medicine::where('commercial_name->'.$request->header('locale'), 'like', '%' . $request->search . '%')
                            ->orWhere('scientific_name->'.$request->header('locale'), 'like', '%' . $request->search . '%')
                            ->get();
            ...
            $categories = Category::where('name->'.$request->header('locale'),'like', '%' . ucfirst($request->search) . '%')->get();
